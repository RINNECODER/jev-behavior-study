...
export class City {
 ...
 step(){
  ...
  for(const other of this.traffic){const u=unit(other.heading),j={x:nearest(other.x),z:nearest(other.z)},axis=Math.abs(u.z)>.5?'z':'x';const dist=(j.x-other.x)*u.x+(j.z-other.z)*u.z;const stop=dist>8&&dist<13&&signal(this.t,j.x,j.z,axis)==='red';if(!stop){other.x+=u.x*other.speed*DT;other.z+=u.z*other.speed*DT;}if(other.x>108)other.x=-108;if(other.z< -108)other.z=108;}
 ...
 observation(){
  ...
  return {units:'meters, seconds, radians; x east, z south; heading 0 north, positive clockwise; right-hand traffic',time:this.t,challenge:this.challenge,vehicle:{...c,heading_name:HEADINGS[h]},speed_limit_mps:10,roads:{centers:ROADS,half_width:7,lane_center_offset:3.5,boundary:LIMIT},next_intersection:j?{...j,signal:signal(this.t,j.x,j.z,axis)}:null,nearby_traffic:this.traffic.filter(o=>Math.hypot(o.x-c.x,o.z-c.z)<55).map(o=>({...o})),destinations:this.challenge==='explore'?[]:this.goals.slice(this.stops),objective:this.challenge==='explore'?'Travel at least 150m and visit two distinct intersections within 90s without a collision.':'Reach each destination and slow below 2 m/s within 6m of it.',previous_controls:{...this.action},actuation:'Steering -1 full left, 0 centered, +1 full right; steering angle=0.5*steer rad. Wheelbase=2.7m. Throttle acceleration=3m/s², brake=7m/s², drag=0.12*speed. Decisions at least 0.6s apart.'};
 }
