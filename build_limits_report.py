"""Generate the decision/order/difficulty report from verified saved metrics."""
import json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
FIRST='20260916T124604860296Z-limits-study'
PIPE='20260916T124845798330Z-decision-pipeline'
STRESS='20260916T125040590877Z-stress-followup'
s=json.loads((ROOT/'results'/PIPE/'limits_summary.json').read_text())

def table(panel,label):
    rows=sorted([r for r in s['tables'] if r['panel']==panel],key=lambda r:r['condition'])
    lines=[f'| {label} | Correct / requests | Accuracy | Mean input tokens/question | Mean output tokens/question |',
        '|---|---:|---:|---:|---:|']
    for r in rows:
        condition=' / '.join(str(x) for x in r['condition'])
        lines.append(f"| {condition} | {r['correct']}/{r['n']} | {100*r['correct']/r['n']:.1f}% | {r['input_tokens_mean']:.2f} | {r['output_tokens_mean']:.2f} |")
    return '\n'.join(lines)

def link(run,name):return f'[results/{run}/{name}](results/{run}/{name})'

text=f'''# Jev: decision workflows, answer order and reasoning limits
...
{table('decision','Stage or method')}
...
{table('position','Correct choice position')}
...
{table('logic_depth','Links in regular chain')}
...
{table('scrambled_logic','Links in underlying scrambled chain')}
...
{table('carry_tracking','People and objects')}
...
{table('dispersed_position','Other records / winning-record position')}
...
- Main phase: {link(FIRST,'manifest.json')}; {link(FIRST,'results.jsonl')};
  {link(FIRST,'per_question.csv')}.
- Pipeline: {link(PIPE,'manifest.json')}; {link(PIPE,'results.jsonl')};
  {link(PIPE,'per_question.csv')}; {link(PIPE,'workflow_usage.csv')}.
- Stress follow-up: {link(STRESS,'manifest.json')}; {link(STRESS,'results.jsonl')};
  {link(STRESS,'per_question.csv')}.
- Machine-readable combined metrics: {link(PIPE,'limits_summary.json')}.
...
python analyze_limits_study.py results/{FIRST} results/{PIPE} results/{STRESS}
...
'''
(ROOT/'DECISION_AND_LIMITS_REPORT.md').write_text(text)
print('Wrote DECISION_AND_LIMITS_REPORT.md')
