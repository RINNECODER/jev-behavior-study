"""Build the research extension from verified metrics and preserved raw evidence."""
import json
from pathlib import Path
from collections import defaultdict
ROOT=Path(__file__).resolve().parent
PILOT='20260916T083425766606Z-everyday-study'
FOLLOW='20260916T085918297474Z-reasoning-followup'

def load(run,name): return json.loads((ROOT/'results'/run/name).read_text())
def table(run):
    rows=load(run,'extension_metrics.json');by=defaultdict(dict)
    for r in rows:
        if r['score_eligible'] and r['family']!='ALL':by[r['family']][r['mode']]=r
    lines=['| Skill | Plain | Reversed choices | Evidence-only prefix |','|---|---:|---:|---:|']
    for family,modes in sorted(by.items()):
        vals=[f"{modes[m]['correct']}/{modes[m]['n']} ({100*modes[m]['correct']/modes[m]['n']:.1f}%)" for m in ['plain','reversed','explicit']]
        lines.append('| '+family.replace('_',' ')+' | '+' | '.join(vals)+' |')
    return '\n'.join(lines)

def tokens(run):
    lines=['| Skill | Mean input tokens/question, plain | Mean input tokens/question, explicit | Mean output tokens/question, plain | Mean output tokens/question, explicit |', '|---|---:|---:|---:|---:|']
    rows=load(run,'extension_metrics.json');families=sorted({r['family'] for r in rows if r['family']!='ALL'})
    for family in families:
        m={r['mode']:r for r in rows if r['family']==family};p=m['plain'];e=m['explicit']
        lines.append(f"| {family.replace('_',' ')} | {p['mean_input_tokens']:.2f} | {e['mean_input_tokens']:.2f} | {p['mean_output_tokens']:.2f} | {e['mean_output_tokens']:.2f} |")
    return '\n'.join(lines)

text=f'''# Jev 1.13.0: everyday reasoning and exactness extension
...
- [Pilot manifest and exact questions](results/{PILOT}/manifest.json)
- [Pilot raw responses](results/{PILOT}/results.jsonl)
- [Pilot per-question usage and scores](results/{PILOT}/per_question.csv)
- [Follow-up manifest and exact questions](results/{FOLLOW}/manifest.json)
- [Follow-up raw responses](results/{FOLLOW}/results.jsonl)
- [Follow-up per-question usage and scores](results/{FOLLOW}/per_question.csv)
...
'''
(ROOT/'EVERYDAY_REASONING_REPORT.md').write_text(text)
print('Wrote EVERYDAY_REASONING_REPORT.md')
